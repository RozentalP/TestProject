...
def face_recognition(path,filename,color='bgr'):   
   # step-1: read image in cv2
    img = cv2.imread(path)
    # step-2: convert into gray scale
    if color == 'bgr':
        ...
        gray = cv2.cvtColor(img,cv2.COLOR_RGB2GRAY)
    # step-3: crop the face (using haar cascase classifier)
    predict2 = haar.detectMultiScale(gray,1.5,3)
    for x,y,w,h in predict2:
        cv2.rectangle(img,(x,y),(x+w,y+h),(255,255,0),2)
    plt.imshow(img)
    
    cv2.imwrite('./static/predict2/{}'.format(filename),img)
